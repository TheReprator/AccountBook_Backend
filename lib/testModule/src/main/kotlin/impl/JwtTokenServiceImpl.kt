...
class JwtTokenServiceImpl(override val jwtConfiguration: JWTConfiguration,
    ...
    override suspend fun generateAccessToken(userId: String, expirationPeriod: Long) = createJWTToken(userId, expirationPeriod)

    override suspend fun generateRefreshToken(userId: String, expirationPeriod: Long) = createJWTToken(userId, expirationPeriod)
...
//        val decodedJwt = jwtVerifier.verify(token) ?: return false
//        if (decodedJwt.expiresAt <= Date())
//            return false
//        val audienceMatches = jwtConfiguration.audience == (decodedJwt.audience.first())
//        if (audienceMatches) {
